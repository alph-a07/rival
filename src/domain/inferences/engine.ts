...
export class AnswerInferenceEngine {
  static evaluate(
    ...
    const applicableRules = rules.filter((rule) =>
      rule.conditions.every((condition) => ConditionEvaluator.holds(condition, context, lookups)),
    );
  ...
  private static sortRules(a: AnswerInferenceRule, b: AnswerInferenceRule): number {
    return resolveEvidence(b).confidence - resolveEvidence(a).confidence;
  }
}

/** Accumulates per-option beliefs via noisy-OR; exclusions hard-zero the option. Consumed by `PresentationPolicy`. */
export class BeliefEngine {
  static evaluate(inferences: EvaluatedAnswerInference[]): Map<string, QuestionBelief> {
...
export class QuestionEvaluationEngine {
  static evaluate(
...
export class ContradictionEngine {
    ...
    const detected = contradictionRules
      ...
      .map((rule) => {
        ...
        return {
          ...
          evidence,
          confidence,
          conflictingAnswers: this.conflictingAnswers(rule),
          explanations: [explanation],
        };
      });
  ...
  private static conflictingAnswers(rule: ContradictionRule): AnsweredOption[] {
...
export class ClarificationEngine {
  static derive(detected: DetectedContradiction[], context: CheckInContext): Clarification[] {
    const clarifications = detected.map((contradiction) => {
      ...
      return {
        ...
        confidence,
        conflictingAnswers,
        reaskQuestionId: this.pickReask(conflictingAnswers, context.answeredSoFar),
        explanations,
      };
    });
...
export class CheckInArchetypeEngine {
  static evaluate(
